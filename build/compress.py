...
def getMessages(lang):
  # Read all messages for this language.
  blocklyMsgFileName = 'appengine/generated/msg/%s.js' % lang
  with open(blocklyMsgFileName, 'r', encoding='utf-8', errors='ignore') as f:
      msgs = f.readlines()
  return msgs
...
def language(gameName, lang):
  ...
  with open('appengine/%s/generated/msg/%s.js' % (gameName, lang), 'w', encoding='utf-8') as f:
    f.write(WARNING)
    if bMsgs:
      f.write('var BlocklyMsg={%s}\n' % ','.join(bMsgs))
    if bgMsgs:
...
def generate_uncompressed(gameName):
  ...
  while directory:
    ...
    # subdir = 'appengine/%s/third-party/' % directory  # Ensure this is added as well
    if os.path.isdir(subdir):
      cmd.append('--root=%s' % subdir)
  ...
  for file in files:
    ...
    if file[:len(prefix)] == prefix:
      file = file[len(prefix):]
        ...
        raise Exception('File "%s" should be in a directory under "%s", but is not.' % (file.replace("\\", "/"), prefix))
